...
\begin{figure}[H]
    ...
    \caption{UMAP projection of 266{,}758 abstracts before clustering.}
    \label{fig:raw_umap}
\end{figure}
...
\section{Results}
In this section, we present the results of our topic modeling analysis of over a quarter million abstracts from various AI related categories on arXiv.

\subsection{Document Landscape Visualization}
We first examine the global landscape of topics produced by the model. 
Topics were identified from all 266,758 AI-related abstracts from 2023-2025 using BERTopic clustering. 
For visualization clarity, Figure~\ref{fig:topic_map} displays a stratified 
sample of 10,000 documents, maintaining proportional representation across 
all discovered topics. The map uses 2-D UMAP coordinates with LLM-generated 
topic labels rendered via DataMapPlot.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{fig_topic_analysis_map.png}
    \caption{Labeled topic landscape from 269,357 arXiv abstracts, visualized using stratified sampling (n=10,000) of UMAP reduced embeddings from BERTopic clustering.}
    \label{fig:topic_map}
\end{figure}
